...
public class Unit {
    ...
    public Unit(String name, int x, int y, UnitType type, UnitJob job, Stats stats, Weapon weapon) {
        this.name = name;
        this.x = x;
        this.y = y;
        this.type = type;
    ...
    public UnitJob getJob() { return job; }
    public MovementType getMovementType() { return job.getMovementType(); }

    public int getMoveRange() { return 5; }
    ...
    public TextureRegion getCurrentFrame() {
        ...
        if (hasMoved) return idleAnimation.getKeyFrames()[0];
    ...
    private void initializeAnimations(String textureName) {
        try {
            ...
            try {
                if (type == UnitType.PLAYER) characterTexture = new Texture(Gdx.files.internal("Knight-Idle.png"));
                else characterTexture = new Texture(Gdx.files.internal("Orc-Idle.png"));
